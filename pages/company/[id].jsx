...
import { getLaunchesByCompanyId, getCompanyById } from '../../utils/api';
...
const Company = ({ company }) => {
    ...
    const handlePaginationChange = ({ page }) => {
        router.push(`/company/${id}/?page=${page}`);
    };

    return (
        <>
            <Head>
                <title>{getTitle(company.name)}</title>
                <meta name="description" content={`Upcoming ${company.name} launches`} />
                <meta property="og:title" content={getTitle(company.name)} />
                <meta property="og:description" content={`Upcoming ${company.name} launches`} />
            ...
            <Box pad={{ horizontal: 'medium' }}>
                ...
                {empty && (
                    <Box align='center' pad={{ bottom: 'large' }}>
                        <Text size='large' color={'status-error'}>No Upcoming {company.name} Launches</Text>
                    </Box>
                )}
                {total > 0 && (
                    <Box align='center' pad={'large'}>
